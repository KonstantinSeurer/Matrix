...
public:
	VulkanStructBuffer(StructBufferType type, MemoryType memoryType) :
		StructBuffer(type, memoryType) {
	}
...
public:
	VulkanArrayBuffer(ArrayBufferType type, MemoryType memoryType, u32 length, VkDevice device) :
		ArrayBuffer(type, memoryType, length), device(device) {
	}
