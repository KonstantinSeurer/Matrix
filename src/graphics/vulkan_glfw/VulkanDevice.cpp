...
namespace matrix
{
	...
	namespace graphics
	{
		...
		VulkanDeviceInstance::VulkanDeviceInstance(VkPhysicalDevice physicalDevice) : physicalDevice(physicalDevice)
		{
...
#define VULKAN_LOAD_FUNCTION(function) function = reinterpret_cast<PFN_##function>(vkGetDeviceProcAddr(device, #function))
		...
		VulkanDevice::VulkanDevice(VkPhysicalDevice device) : device(device)
		{
