...
VulkanSwapchain::VulkanSwapchain(VkPhysicalDevice physicalDevice, VkDevice device, VkQueue queue, VkSurfaceKHR surface,
		GLFWwindow *window, u32 queueFamilyIndex, Ref<VulkanSemaphoreChain> semaphoreChain) :
		device(device), queue(queue), currentImageIndex(0), semaphoreChain(semaphoreChain) {
