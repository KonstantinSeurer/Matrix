...
namespace matrix
{
	...
	namespace scene
	{
		...
		Model::Model(const ModelType *type, Ref<graphics::DeviceInstance> device, const Array<RenderImplementationType> &renderImplementationTypes,
					 const Array<void *> &attributes, u32 vertexCount, const Array<u32> &indices, Ref<const Material> material) : type(type), material(material)
		{
		...
		Model::Model(const Model &model, Ref<const Material> material) : Renderable(model), type(model.type), material(material)
		{
		}

		Model::Model(const Model &model) : Renderable(model), type(model.type), material(model.material)
		{
		}
