...
#include "PbrDeferredPathtracer.h"
...
PbrDeferredPathtracer::PbrDeferredPathtracer(Ref<DeviceInstance> device, Ref<SkyRenderer> skyRenderer) :
...
void PbrDeferredPathtracer::render(Ref<graphics::CommandBuffer> cb, const Scene &scene, ComponentReference camera, u32 bufferIndex)
{
	...
	cb->barrier(
		{ ImageLayoutTransition(gBufferRenderer->getAlbedoMetallic(), ImageLayout::COLOR_ATTACHMENT, ImageLayout::SHADER_READ_ONLY), ImageLayoutTransition(
			gBufferRenderer->getNormalRoughness(), ImageLayout::COLOR_ATTACHMENT, ImageLayout::SHADER_READ_ONLY), ImageLayoutTransition(
			gBufferRenderer->getEmissiveOcclusion(), ImageLayout::COLOR_ATTACHMENT, ImageLayout::SHADER_READ_ONLY), ImageLayoutTransition(
			gBufferRenderer->getDepth(), ImageLayout::DEPTH_ATTACHMENT, ImageLayout::SHADER_READ_ONLY), ImageLayoutTransition(skyRenderer->getEnvironment(),
			ImageLayout::SHADER_STORAGE, ImageLayout::SHADER_READ_ONLY), ImageLayoutTransition(skyRenderer->getTransmittance(), ImageLayout::SHADER_STORAGE,
	...
	cb->barrier(
		{ ImageLayoutTransition(result, ImageLayout::SHADER_STORAGE, ImageLayout::TRANSFER_SOURCE), ImageLayoutTransition(skyRenderer->getEnvironment(),
			ImageLayout::SHADER_READ_ONLY, ImageLayout::SHADER_STORAGE), ImageLayoutTransition(skyRenderer->getTransmittance(), ImageLayout::SHADER_READ_ONLY,
			ImageLayout::SHADER_STORAGE) });
