...
namespace matrix
{
	...
	namespace scene
	{
		...
		DeferredRenderer::DeferredRenderer(Ref<DeviceInstance> device) : device(device)
		{
		...
		void DeferredRenderer::resize(u32 width, u32 height)
		{
			...
			frameBuffer = device->createFrameBuffer(width, height, renderPass, {albedoMetallicAttachmentView, normalRoughnessAttachmentView, emissiveOcclusionAttachmentView, depthAttachmentView});
		}
