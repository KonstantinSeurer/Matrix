...
#define null nullptr
	...
	Ref<Destination> CastUp(const Ref<Source> &ref)
	{
		return std::static_pointer_cast<Destination>(ref);
	}

	template <typename Destination, typename Source>
	Ref<Destination> CastDown(const Ref<Source> &ref)
	{
		return std::dynamic_pointer_cast<Destination>(ref);
	}

	template <typename Destination, typename Source>
	Destination *CastDown(Source *ref)
	{
...
#define MATRIX_HASH_EQUALS(Class) \
	MATRIX_HASH(Class)            \
	MATRIX_EQUAL(Class)
...
#define MATRIX_ASSERT(expression)                                   \
	if (!(expression))                                              \
	{                                                               \
		std::cout << "Assert failed: " << #expression << std::endl; \
		exit(1);                                                    \
	}
